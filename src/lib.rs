...
contract! {
    ...
    impl Erc721 {
        ...
        pub(external) fn set_approval_for_all(&mut self, to: AccountId, approved: bool) -> bool {
            let caller = env.caller();
        ...
        pub(external) fn transfer_from(&mut self, from: AccountId, to: AccountId, id: TokenId) -> bool {
            ...
            if owner == *self.default_account {
                env.println(&format!("Erc721::transfer_from::owner is None"));
                return false;
            }
            if owner != from {
                env.println(&format!("Erc721::transfer_from::from is not owner"));
                return false;
            }
            if to == *self.default_account {
                env.println(&format!("Erc721::transfer_from::spender is None"));
                return false;
            }
        ...
        pub(external) fn approve(&mut self, to: AccountId, id: TokenId) -> bool {
            ...
            if owner == to {
                env.println(&format!("Erc721::approve::approve to current owner"));
                return false;
            }
